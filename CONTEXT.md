...
- **Core** (`lib/governance/`) — `intent-model.js` (classifies a request into
  ...
  declarative BLOCKING/non-blocking definition, never inferred from text), `task-status.js` (a pure function:
  `ACTIVE`/`WAITING_HUMAN_CONFIRMATION`/`READY_TO_CONTINUE`, computed from pending/confirmed decisions, never
  persisted), `agent-context.js`/`task-handoff.js`/`bootstrap.js` (what an external agent actually reads,
  including — since Decision Continuity/Just-In-Time Governance/Governance In-Flow — a "Task Status" and
  "Confirmed decisions" section refreshed the moment `juntia confirm` runs, at any point in a task, not only
  at its start).
- **Knowledge Layer** (`.juntia/governance/`, scaffolded once by `juntia init`, project-editable after that,
  and the ONLY governance source any current code path reads or writes — a project still carrying the
  pre-Phase-15B legacy scheme migrates via `juntia update`,
  `lib/project-intelligence/governance-migration.js`) — `workflows/`, `roles/`, `skills/`, `rules/` (including
  `agent-rules.md`, `decision-triggers.md`, `governance-signals.md`), `conventions/`.
...
Public beta (`0.x`), currently `0.14.0` on npm (Single Governance Source of Truth + Governance In-Flow). See `CHANGELOG.md` for
exactly what shipped in each version.
...
arms), and found several real gaps, closed across three phases: a confirmed decision not reliably reaching the
agent mid-task (`phases/decision-continuity.md`); an external agent's own `pending.json` write being rejected
by a contract Core didn't actually accept, plus `governance-review` asking its questions only once the agent
had practically finished implementing (`phases/just-in-time-governance.md`); and two generations of governance
coexisting with no migration path, plus no deterministic signal that a blocking decision was still unanswered
across a session boundary (`phases/single-governance-source-of-truth.md`). Real known limitations remain,
named plainly in `README.md`: only Claude Code is a built runtime integration, the public API/CLI surface can
still change, and the session has not yet been re-run to check whether any of these fixes hold in practice.

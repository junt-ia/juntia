...
| `juntia analyze` | Understand an existing project: detect technologies, analyze structure, create initial context, surface unknowns, prepare base documentation. | **Designed, not built** | Only for the parts that genuinely need interpretation (see below) — detecting a `package.json`/language/framework is mechanical and would not call AI. |

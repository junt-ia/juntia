...
| `juntia route "<request>"` | Classifies a free-text request into `feature`/`bug`/`investigation`/`refactor`/`unknown` (numeric confidence, never a guess when signal is weak), resolves the matching `.juntia/governance/workflows/*.md` file, prints the Agent Context (`{ task, workflow, roles, skills, contextSources }`), and — only when a real workflow was resolved — writes `.juntia/task-handoff.md` for whichever AI agent reads it next; also refreshes `.juntia/BOOTSTRAP.md`. Scaffolds `.juntia/` first if it doesn't exist yet, same as `integrate`. Not part of the primary `setup`/`analyze`/`integrate` experience an agent following `CLAUDE.md` → `BOOTSTRAP.md` needs — kept as a real, advanced command mainly useful for debugging what Juntia would resolve for a given request, per Phase 15D's own explicit "no eliminar todavía." | **Built** (Phase 15C, extended Phase 15D) | Never — deterministic keyword classification + markdown parsing, no AI call. |
...
entrypoint; every other capability in this table already gets one. `classifyIntent`/`analyzeProduct`/
`analyzeArchitecture`/`analyzeEngineering`/`interpretIntent` remain internal engine — reachable
programmatically via `require('juntia')`, never exposed as a command — and neither is the newer, still-internal
plumbing behind `route` itself (`resolveWorkflowForIntent`, `parseWorkflowMarkdown`); only `classifyTaskIntent`
and `routeWorkflow`, the functions that actually power `route`, are exported. A separate `juntia

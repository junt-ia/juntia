...
process:
  ...
  - If blocking, escalate it as a decision request (`product-decision-making` or `architecture-decision-record`, depending on type) right now — do not keep working on the specific piece of work that depends on the answer. Include a `reason` naming why it's blocking (the matched trigger's own reason, or your own equivalent judgment) so a human — or a later session — sees why, not just what.
  - Immediately after writing the request, run `juntia confirm` — even without a human answer yet, answering `skip` when prompted for this item. This marks `.juntia/task-handoff.md`'s own "Task Status" as `WAITING_HUMAN_CONFIRMATION`, deterministically, for anyone (including a later session) reading the file — no new command, the same `juntia confirm` you'll run again once you actually have the answer.
  - Pause only that piece of work. An independent part of the same task that doesn't depend on this answer can continue.
  - Once a human answers via `juntia confirm`, `.juntia/task-handoff.md` is refreshed automatically — Task Status returns to a normal state and "Confirmed decisions" carries the real answer — re-read it and resume the paused work from there.

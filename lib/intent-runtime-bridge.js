...
// classified Legacy Reasoning Layer since Phase 15A, but — unlike the other
// five legacy modules — still exported on the public API
// (`require('@juntia/juntia').interpretIntent`), the same "reachable by
// something real" test that kept it alive through Phase 13D/15A. Phase 15C
// adds `classifyTaskIntent`/`routeWorkflow` alongside it, not instead of
// it — the two taxonomies (legacy nine-intent AMBIGUOUS-aware bridge vs.
// this phase's four-workflow-shaped classifier) are deliberately not
// unified. See phases/15c-workflow-routing.md.

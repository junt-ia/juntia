...
//
// The rules an interpretation of this project's facts must follow — the
// SECOND interpretation domain this migration has built (the first was
// intent interpretation, Phase 11C). Originally sent as a literal system
// prompt to an internally-spawned AI runtime; since Phase 13D retired that
// internal execution in favor of an AI Handoff (see phases/13d-ai-handoff-
// implementation.md), `SYSTEM_PROMPT` is now reused verbatim inside the
// generated `.juntia/agent-instructions.md` (agent-handoff.js) — same
// rules, same single source of truth, now read by a human's own AI agent
// instead of passed to a subprocess. `OUTPUT_SCHEMA` still documents the
// exact shape a valid interpretation must have, enforced independently by
// `project-interpretation-validator.js` regardless of who produced it.
//

...
// Phase 14A: this file is the governance INDEX, not just a context pointer
// — it names every real, Juntia-maintained file this project has and what
// each one answers, so the connected agent finds all of them from one
// place, without any of their content being copied here. `.juntia/RULES.md`
// is only listed when it exists (human-facing, scaffolded by `init`, not
// generated by this module) — never invented if the project never ran
// `init`, and never claimed to be "generated by Juntia" since a human, not
// Juntia, is its real author.
function buildPointerContent(profile, runtimeName, { hasProjectRules = false } = {}) {
  const lines = [
    GENERATED_MARKER,
...
function integrateRuntime(projectRoot, runtimeName) {
  ...
  const hasProjectRules = fs.existsSync(path.join(projectRoot, '.juntia', 'RULES.md'));
  fs.writeFileSync(targetPath, buildPointerContent(profile, runtimeName, { hasProjectRules }));

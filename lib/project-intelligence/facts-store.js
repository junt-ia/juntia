...
const { RUNTIME_PROFILES } = require('./agent-integration.js');
...
// Phase 13B: real filenames Juntia itself generates as an AI-runtime
// integration (today, only CLAUDE.md — Phase 12L) — never a project's own
// architectural decision, even though the file really exists on disk and a
// deterministic scan would otherwise see no difference between it and
// src/index.js. Derived from RUNTIME_PROFILES (the same single source of
// truth `juntia integrate` already uses to generate these files) rather
// than a second, hand-maintained list — a future real provider profile
// becomes "managed" here automatically, with zero extra code. `.juntia/`
// itself needs no entry: structure-detector.js already excludes the whole
// directory at the scan level (Phase 12I), so it never reaches this point.
const MANAGED_FILENAMES = new Set(Object.values(RUNTIME_PROFILES).map((p) => p.file));
...
function factsFromScanResult(scanResult) {
  ...
  for (const file of scanResult.structure.files) {
    // A real file Juntia generated as an AI-runtime integration (e.g.
    // CLAUDE.md) is real evidence of something — just not of the project's
    // own architecture. Category alone (never silently dropped) makes the
    // distinction: `formatChanges()`'s already-generic
    // `${fact.category}: ${fact.name}` rendering means a diff now correctly
    // reads "Added: managed.file: CLAUDE.md", not "Added: structure.file:
    // CLAUDE.md" as if a human had architected a new project file.
    const category = MANAGED_FILENAMES.has(file) ? 'managed.file' : 'structure.file';
    facts.push({ category, name: file, evidence: { source: 'scan' } });
  }
...
// Ensures a given .juntia/ filename is excluded from the CONSUMING
// project's own git tracking by default — a real, machine-regenerated or
// machine-proposed artifact, not human-authored narrative like
// PROJECT_STATE.md/DECISIONS.md (Phase 12H's own tier distinction). A
// scoped .juntia/.gitignore (not touching the project's own root
// .gitignore) is additive and reversible: a team that deliberately wants to
// version one of these files can delete its one line. Idempotent — never
// overwrites a .gitignore a project already has; appends a new line only if
// that exact filename isn't already listed (Phase 12K reuses this for
// pending.json, extending the file Phase 12I created rather than writing a
// second .gitignore-management routine).
function ensureIgnored(projectRoot, filename) {
  const juntiaDir = path.join(projectRoot, FACTS_DIR);
  const gitignorePath = path.join(juntiaDir, GITIGNORE_FILE);
  fs.mkdirSync(juntiaDir, { recursive: true });
  if (!fs.existsSync(gitignorePath)) {
    fs.writeFileSync(gitignorePath, `${filename}\n`);
    return;
  }
  const existing = fs.readFileSync(gitignorePath, 'utf8');
  if (existing.split('\n').map((l) => l.trim()).includes(filename)) return;
  fs.writeFileSync(gitignorePath, existing.endsWith('\n') ? `${existing}${filename}\n` : `${existing}\n${filename}\n`);
}

// Phase 12I's original, single-purpose entrypoint — kept so every existing
// caller/test keeps working unchanged.
function ensureFactsIgnored(projectRoot) {
  ensureIgnored(projectRoot, FACTS_FILE);
}

...
//
// Contract tolerance (Just-In-Time Governance phase): a real dogfooding
// session (Phase 16B) reproduced this exact failure — an external agent
// following `.juntia/governance/rules/agent-rules.md`'s own documented
// example (a bare decision-request object, never shown wrapped) wrote a
// plain JSON array of decision requests directly to `pending.json`, and this
// function rejected the whole document as unrecognized: "pending.json
// exists but is not a recognized pending document." Juntia declares that an
// external agent may write to this file directly (Phase 13D's own AI
// Handoff model) — a representation the agent can plausibly produce from
// that same declared contract must not be the one thing that then breaks
// the contract. A bare array IS the items list; nothing else a valid
// pending.json could mean starts as a JSON array, so this is unambiguous,
// not a guess. `wasArrayShape` lets `normalizePendingItems` below
// self-heal the file back to the canonical `{ schemaVersion, items }` shape
// the moment it's next touched — `savePending` never writes the array
// shape, only ever reads it.
function loadPending(projectRoot) {
  ...
  if (Array.isArray(document)) {
    return {
      exists: true, unknown: false, items: document, wasArrayShape: true,
    };
  }
...
function normalizePendingItems(projectRoot) {
  ...
  // A bare-array pending.json (see `loadPending` above) is always rewritten
  // canonically the first time it's touched, even if every item already has
  // a real id/status/createdAt — `savePending` never writes the array shape,
  // so leaving it on disk would mean every future read keeps depending on
  // this same tolerance instead of converging on one real shape.
  let changed = Boolean(loaded.wasArrayShape);
  const used = new Set();

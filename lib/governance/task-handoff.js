...
const { ensureIgnored } = require('../project-intelligence/facts-store.js');
...
// Hardening phase — task-handoff.md's own git lifecycle, audited alongside
// the confirm-channel fix above (same M04 v2 dogfooding session): is this
// file regenerable machine state, like facts.json/pending.json (both
// .gitignored by default via this exact `ensureIgnored` call — see
// facts-store.js's own header), or durable, human-authored narrative, like
// DECISIONS.md/decisions.json (deliberately never git-ignored)?
//
// It is regenerable. Every section of its content — Task Status, Confirmed
// decisions, Potential decisions, the embedded Agent Context — is computed
// fresh, on every write, from `decisions.json`/`pending.json`/the Knowledge
// Layer/the route just resolved; nothing in this file is itself a source of
// new information the way a human's answer in decisions.json is. Even the
// two inputs that AREN'T re-derivable from another file on disk — the
// original request `text` and the `generatedAt` timestamp, both recovered
// from this file's own `<!-- juntia:task-meta -->` comment by
// `refreshTaskHandoffDecisions` above — are not lost information if this
// file is: they are exactly what a fresh `juntia route "<same request>"`
// call re-supplies, the same "just re-run the command that made it" recovery
// `pending.json` already has (an agent re-proposes; nothing is unrecoverable
// the way a human's typed decision would be). It is also, by construction,
// PER-TASK and PER-CONTRIBUTOR — the CURRENT request someone is routing
// right now, wholesale-overwritten by the next `juntia route` call — the
// same reason committing it would mean noisy, purely-local-state diffs on
// every branch, never real project history the way DECISIONS.md is.
function ensureTaskHandoffIgnored(projectRoot) {
  ensureIgnored(projectRoot, path.basename(TASK_HANDOFF_FILE));
}

function writeTaskHandoff(projectRoot, markdown) {
  ensureTaskHandoffIgnored(projectRoot);

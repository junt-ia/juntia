...
// buildTaskHandoff(text, route, { decisions, generatedAt }) -> markdown
// string, or null when `route` has no resolved workflow. Per this phase's
// own explicit "una petición ambigua nunca debe producir un workflow
...
// later regeneration instead of resetting it.
function buildTaskHandoff(text, route, options = {}) {
  if (!route || !route.workflow) return null;
  const { decisions = [], generatedAt = new Date().toISOString() } = options;
...
// refreshTaskHandoffDecisions(projectRoot, decisions) -> path | null
//
...
// changes which workflow/roles/skills apply), fresh `decisions`. This is
// the one piece of state that makes a confirmed decision an "instruction for
// unfinished work," not just history: the agent's own active task file now
// carries it, flagged as new, without needing to have been present when
// `confirm` ran or to remember a prior conversation turn. Never touches
// source code — this only ever rewrites Juntia's own generated navigation
// file, the same class of operation `writeContext`/`writeBootstrap` already
// are. A pre-phase task-handoff.md (no recoverable state) or none at all is
// left exactly as-is — not a regression, just nothing new to refresh yet.
function refreshTaskHandoffDecisions(projectRoot, decisions) {
  const targetPath = path.join(projectRoot, TASK_HANDOFF_FILE);
  ...
  const markdown = buildTaskHandoff(state.text, state.route, { decisions, generatedAt: state.generatedAt });

...
// request gets solved."
//
// Deliberately NOT the same taxonomy as the legacy `lib/intent-router.js`
// (nine intents, string confidence). That module's own free-text detection
// half is documented, since Phase 15A, as duplicated reasoning an external
// agent already does natively — this module does not extend it or reuse its
// rules; it is a new, smaller, four-workflow-shaped classifier built to
// match the real Knowledge Layer workflow files Phase 15B shipped
...
// the same sentence — the same real, corpus-evidenced pattern
// `lib/intent-router.js`'s own BUG rule already established (see that
// module's own comment on this), reused here as grounding for a fresh rule,
// not copied code.
const RULES = [
  {
    intent: 'bug',

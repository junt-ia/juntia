...
function buildBootstrap(projectRoot) {
  ...
  lines.push(
    ...
    'The moment one does, follow `.juntia/governance/skills/governance-review/SKILL.md`: escalate it, pause',
    'only the specific piece of work that depends on the answer, and keep going on anything that does not.',
    'Once a human answers via `juntia confirm`, re-read `.juntia/task-handoff.md` — it is refreshed',
    'automatically with the confirmed decision — and resume from there. Do not try to predict every decision',
    'a feature will eventually need before you begin.',
  );
  ...
  if (hasActiveTaskHandoff) {
    lines.push(
      ...
      '',
      'If you are resuming mid-task, re-read its own "Confirmed decisions" section before continuing —',

...
// buildAgentContext(route, { blockingPending, decisions, generatedAt }) -> {
...
//   taskStatus: 'ACTIVE' | 'WAITING_HUMAN_CONFIRMATION' | 'READY_TO_CONTINUE',
// }
//
...
//
// `taskStatus` (Governance In-Flow phase) — the one field here that IS a
// real, computed state rather than pure navigation, deliberately still not
// a "solution": it says whether work is currently blocked, never what to do
// about it. `blockingPending`/`decisions`/`generatedAt` are optional and
// defensively defaulted (empty/`[]`/`undefined`) so every pre-existing
// caller — including every test written before this phase — gets
// `taskStatus: 'ACTIVE'` unchanged, the same value the old, two-argument
// call shape always implied. See `task-status.js` for the actual rule.
function buildAgentContext(route, { blockingPending = [], decisions = [], generatedAt } = {}) {
  ...
  return {
    ...
    skills: route.skills,
    contextSources,
    taskStatus: computeTaskStatus({ blockingPending, decisions, generatedAt }),
  };
}
